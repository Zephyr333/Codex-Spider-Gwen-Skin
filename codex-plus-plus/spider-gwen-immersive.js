...
/*
 * Continue each route's existing material through Electron's 36px native menu
 * row. This deliberately adds no blur, shadow, border or extra texture, so the
 * wallpaper and Gwen's vertical silhouette remain visually continuous.
 */
html.codex-dream-skin.dream-art-wide.codex-spider-gwen-immersive body::before {
  content: "";
  position: fixed;
  inset: 0 0 auto 0;
  ...
  pointer-events: none;
  background:
    linear-gradient(90deg, rgb(6 17 35 / .82), oklab(0.214349 -0.00978811 -0.0490709 / .82)) 0 0 / var(--sg-shell-left, 275px) 100% no-repeat,
...
/* Codex 1.2.42 inserts a home-banners host before the actual homepage
   content. Dream Skin v3's legacy first-child rule expands that host to a
...
html.codex-dream-skin.dream-art-wide.codex-spider-gwen-immersive [data-spider-gwen-role="home-composer-group"] {
  ...
  background: var(--sg-home-composer-base) !important;
  box-shadow:
    0 16px 36px var(--sg-shadow),
    inset 0 1px 0 var(--sg-material-highlight),
    inset 0 -1px 0 var(--sg-material-depth) !important;
...
html.codex-dream-skin.dream-art-wide.codex-spider-gwen-immersive [data-spider-gwen-role="home-composer-group"]::after {
  ...
  pointer-events: none;
  opacity: 0;
  background: var(--sg-focus-ring-both);
  -webkit-mask:
    linear-gradient(#000 0 0) content-box,
    linear-gradient(#000 0 0);
  -webkit-mask-composite: xor;
...
`;
  ...
  const clearRouteClasses = () => {
    ...
    document.querySelectorAll(`.${HOME_COMPOSER_GROUP_CLASS}`).forEach((node) => node.classList.remove(HOME_COMPOSER_GROUP_CLASS));
  ...
  const scheduleEnsure = (reason = "unknown") => {
    ...
    scheduler.settleTimeout = setTimeout(() => {
      scheduler.settleTimeout = null;
      ensure();
    }, 900);
  };

  const watchShellGeometry = (shellMain) => {
    if (observedShellMain === shellMain) return;
